...
import { FormControlProps, FormControlSchema } from './Item';
 ...
 */
export interface ArrayControlSchema extends Omit<ComboControlSchema, 'type' | 'controls' | 'conditions'> {
    /**
     * 指定为数组输入框类型
     */
    type: 'array';
...
export interface ArrayProps extends FormControlProps, Omit<ArrayControlSchema, 'type' | 'className' | 'descriptionClassName' | 'inputClassName'> {
    store: IComboStore;
}
